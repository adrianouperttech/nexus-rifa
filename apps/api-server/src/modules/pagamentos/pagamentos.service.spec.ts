...
describe('PagamentosService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PagamentosService,
        { provide: getRepositoryToken(Pagamento), useFactory: () => ({ save: jest.fn(), findOne: jest.fn(), create: jest.fn() }) },
        { provide: ReservasService, useFactory: () => ({ findOne: jest.fn(), updateStatus: jest.fn() }) },
  ...
  describe('create', () => {
    ...
    it('should throw ConflictException if reserva is not disponivel', async () => {
      jest.spyOn(reservasService, 'findOne').mockResolvedValue({ status: 'paga' } as any);
      await expect(service.create(tenant_id, createPagamentoDto)).rejects.toThrow(ConflictException);
    });
  ...
  describe('handlePagamentoWebhook', () => {
    it('should update reserva to confirmada if payment is approved', async () => {
      ...
      expect(pagamentoRepository.save).toHaveBeenCalledWith({ ...pagamento, status: 'pago' });
    ...
    it('should update reserva to disponivel if payment is cancelled', async () => {
        ...
        expect(pagamentoRepository.findOne).toHaveBeenCalledWith({ where: { transacao_id: '12345' }, relations: ['reserva'] });
        expect(pagamentoRepository.save).toHaveBeenCalledWith({ ...pagamento, status: 'cancelado' });
